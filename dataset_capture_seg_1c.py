...
def main(config,cu_seed,frame_no,hour,human_waypoints,scans_cap):
    ...
    try:
       ...
       #### RGB Cameras ####
        ...
        #Configure camera parameters
        camera_bp_3.set_attribute('fov',str(60)) #In cm
        ...
        #Configure camera parameters
        camera_bp_4.set_attribute('fov',str(60)) #In cm
        ...
        #Configure camera parameters
        camera_bp_5.set_attribute('fov',str(60)) #In cm
        ...
        camera_transform2 = carla.Transform(carla.Location(x=y1,y=x1+ offset, z=z), carla.Rotation(yaw=-pitchdegrees))
        camera_transform3 = carla.Transform(carla.Location(x=y1 ,y=-(x1+ offset), z=z), carla.Rotation(yaw=pitchdegrees))
        camera_transform4 = carla.Transform(carla.Location(x=y2,y=x2 + offset, z=z), carla.Rotation(yaw=-2*pitchdegrees))
        camera_transform5 = carla.Transform(carla.Location(x=y2,y=-(x2 + offset), z=z), carla.Rotation(yaw=2*pitchdegrees))
        camera_rgb_1 = SpawnActor(camera_bp, camera_transform, vehicle)#world.spawn_actor(
            #camera_bp,
            #camera_transform,
            #attach_to=vehicle)
        ...
        ss_camera_1 = SpawnActor(ss_camera_p,camera_transform,vehicle) #world.spawn_actor(
        #    ss_camera_p,
        #    camera_transform,
        #    attach_to=vehicle)
        #ss_camera_2 = world.spawn_actor(
        #    ss_camera_p,
        #    camera_transform2,
        #    attach_to=vehicle)
        #ss_camera_3 = world.spawn_actor(
        #    ss_camera_p,
        #    camera_transform3,
        #    attach_to=vehicle)
        #ss_camera_4 = world.spawn_actor(
        #    ss_camera_p,
        #    camera_transform4,
        #    attach_to=vehicle)
        #ss_camera_5 = world.spawn_actor(
        #    ss_camera_p,
        #    camera_transform5,
        #    attach_to=vehicle)
        
        
        sensor_list.append(ss_camera_1)
        #sensor_list.append(ss_camera_2)
        #sensor_list.append(ss_camera_3)
        #sensor_list.append(ss_camera_4)
        #sensor_list.append(ss_camera_5)
        cameras = []
        for response in client.apply_batch_sync(sensor_list, synchronous_master):
            if response.error:
                print("Spawn Error")
            else:
                cameras.append(response.actor_id)
                actor_list.append(response.actor_id)

        camera_rgb_1 = world.get_actors().find(cameras[0])
        ss_camera_1 = world.get_actors().find(cameras[1])
        ...
        ##############################################################################################
        with CarlaSyncMode(world, camera_rgb_1,ss_camera_1, fps=fps) as sync_mode:
            counter = 0
            step = 0
            while True:
                step+=1
                ...
                snapshot, rgb_1,ss_1 = sync_mode.tick(timeout=50.0) #Ajusta timeout si el pc es muy lento
                ...
                if( save == 1):
                    ...
                    rgb_1.save_to_disk(dataset_path+('cam1/rgb/{}_{}.png').format(step//50+frame_no,i)) # Save the scan
                    ...
                    ss_1.save_to_disk(dataset_path+('cam1/ss/{}_{}.png').format(step//50+frame_no,i)) # Save the scan
                    #ss_2.save_to_disk(dataset_path+('cam2/ss/{}.png').format(ss_2.frame)) # Save the scan
                    #ss_3.save_to_disk(dataset_path+('cam3/ss/{}.png').format(ss_3.frame)) # Save the scan
                    #ss_4.save_to_disk(dataset_path+('cam4/ss/{}.png').format(ss_4.frame)) # Save the scan
                    #ss_5.save_to_disk(dataset_path+('cam5/ss/{}.png').format(ss_5.frame)) # Save the scan
